...
`runtimes.spartan.binary` configures only the wrapper executable. SPARTAN itself adds its bounded
runtime context and project-pinned MCP server to every launch. Conductor continues to own only its
fleet protocol, Codex settings, and isolated runtime home, so direct and managed SPARTAN sessions
have the same platform features.
...
mode, and room membership, speech, and teardown. It also routes spawn/teardown, raw terminal input, sentinel assignment, and fleet-watch
control. Exposure is enforced for operations targeting existing sessions, so `all`, broadcasts,
listings, status, room fan-out, raw input, teardown, and sentinel assignment cannot reveal or affect hidden
...
Operator escalation, identity, documentation, room creation, room listing, and federation discovery remain local-only.
