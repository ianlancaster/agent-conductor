...
`systemPromptFile`, while both adapters silently skip some invalid configured sources instead of
preparing one validated, immutable instruction snapshot.
...
- Claude Code compaction reuses the launch system-prompt layer. A disposable Claude Code 2.1.220
  spike on 2026-07-27 verified that a marker supplied by repeated
  `--append-system-prompt-file` arguments remained model-visible after manual `/compact` without a
  reinjection hook. Adding one would duplicate the protocol and session layer.
...
4. let the adapter validate its complete rendered context before changing prior snapshots;
5. write atomic conductor-owned snapshots under the session’s existing `identity.configDir`; and
6. return distinct protocol and session layers rather than one ambiguous concatenated string.
...
- rely on Claude Code's supported reuse of launch system-prompt layers;
- keep the existing lifecycle relay so core health still receives compact-complete evidence; and
- do not add a second `SessionStart` context hook, which would duplicate both prepared layers.
...
Ratified first-release decision: observability metadata, events, and database changes are deferred.
They cannot prove that a runtime accepted or followed the restored context, and are not required to
deliver the mechanical guarantee.
...
Recorded 2026-07-27 evidence:

- Claude Code 2.1.220 applied distinct launch prompt files, retained the generic session marker
  after manual `/compact`, and required no additional context hook. Adding one would duplicate the
  retained layers.
- Codex 0.144.6 displayed its native four-hook review gate. After explicit trust, it applied the
  generic session marker at startup and restored it once through compact `SessionStart`; restoration
  still succeeded with the disposable lifecycle endpoint unavailable.
- Neither runtime received terminal input from Conductor during compaction. Automatic compaction
  was not claimed because neither tested CLI exposed a deterministic, bounded trigger for this
  disposable check; the manual shakedown records it when a future runtime does.
...
- Keep Claude's existing lifecycle settings unchanged; its prepared launch system-prompt snapshots
  are the provider-supported compaction path.
...
### Phase 3 — Truthful observability (deferred)

No status fields, events, or persistence changes ship in the first release. Reconsider only if a
future consumer needs metadata that can be reported without implying model compliance.
...
- fleet-root-relative and absolute paths;
...
| Persistence             | None; prepared files are replaceable runtime artifacts, not database state             |
| Events                  | No new event; lifecycle evidence remains unchanged                                     |
...
- metadata-only instruction provenance and restoration-attempt observability.
...
7. status and events make no new claim about model acceptance or compliance;
