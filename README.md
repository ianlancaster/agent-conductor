...
| Close or rebind unresolved work              | `/close-status <work-id> <reason>` · `/rebind-status <work-id> <reason>`        |
...
by the operator or a configured authority for that worker. Acceptance names the exact claim
