...
and only the launcher path under `runtimes.spartan.binary`. SPARTAN supplies its platform context
and project-pinned MCP tools for each launch. Conductor supplies its normal fleet protocol and
isolated runtime home without duplicating or rewriting SPARTAN configuration.
