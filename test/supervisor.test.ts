...
describe('Supervisor construction', () => {
  ...
  it.each([
    { mode: 'auto=false', auto: false, paused: false, sentinel: false },
    { mode: 'auto=false', auto: false, paused: false, sentinel: true },
    { mode: 'paused', auto: false, paused: true, sentinel: false },
    { mode: 'paused', auto: false, paused: true, sentinel: true },
    { mode: 'auto=true', auto: true, paused: false, sentinel: false },
    { mode: 'auto=true', auto: true, paused: false, sentinel: true },
  ])('notifies an owner for persisted uncertainty in $mode mode (sentinel=$sentinel)', async (scenario) => {
    const port = await freePort();
    writeConfig(
      `mcp:\n  port: ${String(port)}\nmessaging:\n  queueDrainMs: 5\n${scenario.sentinel ? 'sentinel:\n  codename: watch\n' : ''}`,
      {
        alpha: `codename: alpha\nrepo: ${baseDir}\n`,
        ...(scenario.sentinel ? { watch: `codename: watch\nrepo: ${baseDir}\n` } : {}),
      },
    );
    const seed = new Store(join(baseDir, 'data', 'conductor.db'));
    seed.upsertSessionState({
      ...
      pausedAt: scenario.paused ? '2026-09-17T04:00:00.000Z' : null,
      activeRuntime: 'claude-code',
      activeEffort: null,
      activity: 'stopped',
    });
    const receipt = seed.insertDirectMessage('source', 'alpha', 'ambiguous').row;
    seed.markMessageSubmissionStarted(receipt.id);
    seed.close();
    const channel = new ControlledChannel('owner');
    supervisor = new Supervisor(baseDir, {
      terminalBackend: new FakeTerminalBackend(),
      runtimes: [new FakeRuntime('claude-code')],
      channels: [channel],
    ...
    expect(channel.sent[0]?.text).toContain(`message #${String(receipt.id)} to alpha`);
    await new Promise<void>((resolve) => setTimeout(resolve, 20));
    expect(channel.sent).toHaveLength(1);
    const persisted = new Store(join(baseDir, 'data', 'conductor.db'));
    expect(persisted.getMessage(receipt.id)?.uncertain_notified_at).toBeTypeOf('string');
    persisted.close();
  });

  it('retains a startup uncertainty notice through delayed and failing channel delivery', async () => {
    const port = await freePort();
    writeConfig(`mcp:\n  port: ${String(port)}\nmessaging:\n  queueDrainMs: 5\n`, {
      alpha: `codename: alpha\nrepo: ${baseDir}\n`,
    });
    ...
    await supervisor.start();
    expect(channel.sent).toEqual([]);
    let persisted = new Store(join(baseDir, 'data', 'conductor.db'));
    expect(persisted.getMessage(receipt.id)?.uncertain_notified_at).toBeNull();
    persisted.close();
    ...
    persisted = new Store(join(baseDir, 'data', 'conductor.db'));
    expect(persisted.getMessage(receipt.id)?.uncertain_notified_at).toBeTypeOf('string');
    persisted.close();
  });
