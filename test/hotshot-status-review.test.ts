...
it('keeps a key first supplied on an exact duplicate, after subsequent transitions', () => {
  store.workStatus.report(fleet, 'alpha', working, '1', 100);
  store.workStatus.report(fleet, 'alpha', waiting, '1', 200);
  const duplicate = { ...waiting, idempotencyKey: 'retry-duplicate' };
  const receipt = store.workStatus.report(fleet, 'alpha', duplicate, '1', 300);
  store.workStatus.report(fleet, 'alpha', working, '1', 400);
  const retry = store.workStatus.report(fleet, 'alpha', duplicate, '1', 500);
...
it('stops blocked time when the operator resolves the blocker', () => {
  store.workStatus.report(fleet, 'alpha', working, '1', 100);
  store.workStatus.report(fleet, 'alpha', blocker, '1', 200);
  ...
  store.workStatus.report(fleet, 'alpha', working, '1', 1000);
