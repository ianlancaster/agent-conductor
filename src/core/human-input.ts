 ...
 * Facilitated agents ask the operator directly (buttons when the channel has
 * them); autonomous agents ask the sentinel, which answers or escalates.
 */
export class HumanInputBroker {
  ...
  async request(agent: string, question: string, context?: string, options?: string[]): Promise<string> {
    const id = this.nextId;
    this.nextId += 1;

    const answer = new Promise<string>((resolve) => {
      this.pending.set(id, { id, agent, question, options, resolve });
    });

    const sentinel = this.deps.sentinelCodename();
    const routeToSentinel =
      this.deps.getAutonomy(agent) === 'autonomous' &&
      sentinel !== undefined &&
      sentinel !== agent &&
      this.deps.isActive(sentinel);

    if (routeToSentinel) {
      const lines = [`[HumanInput #${id} from ${agent}] ${question}`];
      ...
      const buttons =
        options !== undefined && options.length > 0
          ...
          : undefined;
      const lines = [`❓ *${agent}* asks:`, question];
    ...
    log().info('human-input', `${agent}: question #${id} pending (${routeToSentinel ? 'sentinel' : 'operator'})`);
    return answer;
  }

  /** Resolve a pending question with free-text. Returns the asking agent, or undefined if unknown id. */
  ...
  listPending(): { id: number; agent: string; question: string }[] {
    return [...this.pending.values()].map(({ id, agent, question }) => ({ id, agent, question }));
  }
}
